    ...
    internal class AdvancedMimicUiPatch
    {
        ...
        private class AllConstructDecorations_AddOrEditDecorations_Patch
        {
            private static bool Prefix(AllConstructDecorations __instance, Vector3i position)
            {
                ...
                {
                    new HuwaMultiDecorationUi(new DecorationsSubject(__instance, position, decorationsAtPosition)).ActivateGui(GuiActivateType.Stack);
                }
                else
                {
                    if (__instance.CanAddHere(position))
                    {
                        new HuwaMultiDecorationUi(new DecorationsSubject(__instance, position, null)).ActivateGui(GuiActivateType.Stack);
                    }
                }
