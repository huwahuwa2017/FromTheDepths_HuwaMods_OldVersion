...
{
    public class HuwaMeshSelectTab : SuperScreen<int>
    {
        ...
        public HuwaMeshSelectTab(ConsoleWindow window) : base(window, default)
        {
        }
        ...
        public override void Build()
        {
            ...
            screenSegment_0.AddInterpretter(SubjectiveDisplay<int>.Quick(_focus, M.m(
                (int I) =>
                {
                    ...
                    {
                        return $"Currently selected mesh  :  <<{HuwaHelpFunctions.GetItemName(data.MeshGuid)}>>";
                    }
            ...
            screenSegment_0.AddInterpretter(SubjectiveButton<int>.Quick(
                ...
                (int I) =>
                {
                    if (GetCommonData(out MimicAndDecorationCommonData data))
                    {
                        data.MeshGuid = HuwaHelpFunctions.GetMirrorMesh(data.MeshGuid);
                    }
                }
                )).FadeOut = M.m(
                (int I) =>
                {
                    if (GetCommonData(out MimicAndDecorationCommonData data) && HuwaHelpFunctions.GetItemDefinition(data.MeshGuid, out ItemDefinition selectItem))
                    {
            ...
            screenSegment_0.AddInterpretter(new HuwaSearcher<ModComponentAbstract>(() => "Filter :", () => new ToolTip("Select a mesh to use"),
                ...
                (ModComponentAbstract mca, string filter) =>
                {
                    ...
                    bool flag_0 = HuwaSettingsTab.SearchName && mca.ComponentId.Name.ToLower().Contains(filter);
                    bool flag_1 = HuwaSettingsTab.SearchRuntimeId && mca.ComponentId.RuntimeId.ToString().ToLower().Contains(filter);
                    bool flag_2 = HuwaSettingsTab.SearchGuid && mca.ComponentId.Guid.ToString().ToLower().Contains(filter);
