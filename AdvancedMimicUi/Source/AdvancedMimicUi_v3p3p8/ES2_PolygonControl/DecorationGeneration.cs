...
namespace ES2_PolygonControl
{
    public static class PolygonGeneration
    {
        ...
        public static void Generate(MimicAndDecoration_CommonData MAD_CommonData, PolygonData polygonData)
        {
            int colorIndex = -1;
            ...
            switch (polygonData.PolyType)
            {
                case PolygonType.RightTriangle:
                    ...
                    InputMAD_Data(MAD_CommonData, SBGUID.Slope, position, scare, angles, colorIndex);
                    break;
                case PolygonType.IsoscelesTriangle:
                    ...
                    InputMAD_Data(MAD_CommonData, SBGUID.Wedge, position, scare, angles, colorIndex);
                    break;
                ...
                case PolygonType.Rectangle:
                    ...
                    InputMAD_Data(MAD_CommonData, SBGUID.Block, position, scare, angles, colorIndex);
                    break;
                case PolygonType.Ellipse:
                    ...
                    InputMAD_Data(MAD_CommonData, SBGUID.Pole, position, scare, angles, colorIndex);
                    break;
        ...
        private static void InputMAD_Data(MimicAndDecoration_CommonData data, Guid guid, Vector3 position, Vector3 scare, Vector3 angles, int colorIndex)
        {
            data.MeshGuid = guid;
            data.Positioning = new Vector3(Rounding.R4(position.x), Rounding.R4(position.y), Rounding.R4(position.z));
            data.Scaling = new Vector3(Rounding.R4(scare.x), Rounding.R4(scare.y), Rounding.R4(scare.z));
            data.Orientation = new Vector3(Rounding.R4(angles.x), Rounding.R4(angles.y), Rounding.R4(angles.z));
