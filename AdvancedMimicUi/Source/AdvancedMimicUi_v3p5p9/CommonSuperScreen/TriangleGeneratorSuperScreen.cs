...
namespace AdvancedMimicUi
{
    public class TriangleGeneratorSuperScreen : SuperScreen<int>
    {
        ...
        private List<PolygonData> polygonDataList = new List<PolygonData>();
        ...
        public override void Build()
        {
            ...
            subjectiveFloats[0] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[0].x), "CoordinateA.x", (byte m, float f) => ChangeCoordinate(0, "x", f), new ToolTip("Adjust the left right positioning of the CoordinateA"));
            subjectiveFloats[1] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[0].y), "CoordinateA.y", (byte m, float f) => ChangeCoordinate(0, "y", f), new ToolTip("Adjust the up down positioning of the CoordinateA"));
            subjectiveFloats[2] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[0].z), "CoordinateA.z", (byte m, float f) => ChangeCoordinate(0, "z", f), new ToolTip("Adjust the forward backward positioning of the CoordinateA"));

            subjectiveFloats[3] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[1].x), "CoordinateB.x", (byte m, float f) => ChangeCoordinate(1, "x", f), new ToolTip("Adjust the left right positioning of the CoordinateB"));
            subjectiveFloats[4] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[1].y), "CoordinateB.y", (byte m, float f) => ChangeCoordinate(1, "y", f), new ToolTip("Adjust the up down positioning of the CoordinateB"));
            subjectiveFloats[5] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[1].z), "CoordinateB.z", (byte m, float f) => ChangeCoordinate(1, "z", f), new ToolTip("Adjust the forward backward positioning of the CoordinateB"));

            subjectiveFloats[6] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[2].x), "CoordinateC.x", (byte m, float f) => ChangeCoordinate(2, "x", f), new ToolTip("Adjust the left right positioning of the CoordinateC"));
            subjectiveFloats[7] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[2].y), "CoordinateC.y", (byte m, float f) => ChangeCoordinate(2, "y", f), new ToolTip("Adjust the up down positioning of the CoordinateC"));
            subjectiveFloats[8] = SubjectiveFloatClampedWithBarFromMiddle<byte>.Quick(default, -15f, 15f, 0.05f, 0f,
                M.m((byte I) => Coordinates[2].z), "CoordinateC.z", (byte m, float f) => ChangeCoordinate(2, "z", f), new ToolTip("Adjust the forward backward positioning of the CoordinateC"));

            Array.ForEach(subjectiveFloats,
                (SubjectiveFloatClampedWithBarFromMiddle<byte> I) =>
                {
            ...
            SubjectiveToggle<byte> subjectiveToggle = SubjectiveToggle<byte>.Quick(default, "Normal reversal", new ToolTip("Set the direction to shift the face"),
                (byte I, bool b) =>
                {
                    MADCD_Generation.NormalReversal = b;
                },
                (byte I) =>
                {
                    return MADCD_Generation.NormalReversal;
                });

            screenSegment_1.AddInterpretter(new DropDown<byte, StructureBlockType>(default, dropDownMenuAlt, (i, I) => MADCD_Generation.SBType.Equals(I), (i, I) => MADCD_Generation.SBType = I));
            screenSegment_1.AddInterpretter(SimpleFloatInput<byte>.Quick(default, M.m<byte>(I => MADCD_Generation.FaceThickness), new ToolTip("Set the thickness of the triangle to be generated"), (i, f) => MADCD_Generation.FaceThickness = Mathf.Clamp(f, 0.05f, 10f), M.m<byte>(I => "Thickness ")));
        ...
        public override void LateUpdateWhenActive()
        {
            ...
            Vector3 vector_A = wp + wr * Coordinates[0];
            Vector3 vector_B = wp + wr * Coordinates[1];
            Vector3 vector_C = wp + wr * Coordinates[2];
        ...
        private void TriangleGeneration(bool flip = false)
        {
            ...
            {
                MADCD_Generation.ColorSetting = null;

                if (polygonDataList.Count == 2)
                {
                    MADCD_Generation.Generate(data, polygonDataList[(flip) ? 1 : 0]);
                }
                else
                {
                    MADCD_Generation.Generate(data, polygonDataList[0]);
                }
            }
        }
    }
